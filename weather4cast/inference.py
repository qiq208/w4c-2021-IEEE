...
def load_models_and_weights(id_region, model_location, device=None):
    ...
    """
    #print(device + 'model')
    model = Net1(in_channels=8*4+3,out_channels=4*32).to(device)
...
def get_preds(model, batch, device=None):
    ...
    with torch.no_grad():
        ...
        y_hat = model(in_seq)
        y_hat = y_hat[0].squeeze()
    
    #y_hat = y_hat.data.cpu().numpy()  
    
    return y_hat, day_in_year

def predictions_per_day(test_dates, models, ds_iterator, device, file_path, data_params):
    """ Computes predictions of all dates and saves them to disk """
    for target_date in test_dates:
        batch = next(ds_iterator)
        Ys = []
        for model in models:
            print(f'generating submission for date: {target_date}, with model: {model}')
            y_hat, predicted_day = get_preds(model, batch, device)
            Ys.append(y_hat)
        #y_hat = Ys[0]
        if len(Ys)>1:
            y_hat = torch.mean(torch.stack(Ys), dim=0).cpu().numpy()
        else:
            y_hat = Ys[0].cpu().numpy()
        print(y_hat.shape)
...
# 5. Main program
# ------------
def inference(data_p, region, weights_folder, weights, output, device):
    ...
    for model in weights:
        models.append(load_models_and_weights(region, os.path.join(weights_folder, model), device=device))
    ...
    # ------------
    #if len(models)==1:  
    predictions_per_day(test_dates, models, ds_iterator, device, out_path, params['data_params'])
    #else:
...
def set_parser():
    ...
    parser.add_argument("-f", "--weights_folder", type=str, required=True,
        help='path to folder containing the model weights')
    parser.add_argument("-w", "--weights", nargs="+", type=str, required=True,
        help='list of weights to use')
    ...
    parser.add_argument("-g", "--device", type=str, required=False, default='cpu', 
        help="which device to use - use 'cuda' for gpu. Default is 'cuda' ")
...
def main():
    ...
    inference(options.data, options.region, options.weights_folder, options.weights, options.output, options.device)
...
if __name__ == "__main__":
    ...
    OUT_PATH=.
    python weather4cast/inference.py -d $INPUT_PATH -r $R -f $WEIGHT_FOLDER -w A-o $OUT_PATH -g 'cuda'
